...
router.post('/api/jobseekersignup', async (req, res) => {
    try {
        let item = {
            name: req.body.name,
...
router.get('/api/jobseekerlist', async (req, res) => {
    try {
...
router.get('/api/jobseekers', async (req, res) => {       // getdata for admin to collect unverified jobseeker
    try {
...
router.get('/api/jobseekerVer', async (req, res) => {       // getdata for admin to collect verified jobseeker
    try {
...
router.post('/api/singlejobseeker', async (req, res) => {       // get single data of jobseeker

    try {
        let data = await jobseekerData.findOne({ email: req.body.email, password: req.body.password, approval_status: "verified" });
        let payload = { 'email': req.body.email, 'password': req.body.password, 'date': Date.now() };
        let token = jwt.sign(payload, 'secretkey');
        if (!data) {
            return res.json({ message: " Admin did not verify your data yet !!" });
...
router.get('/api/jobseeker/:id', (req, res) => {
    try {
...
router.put('/api/generaldata', async (req, res) => { // update one jobseeker general information
    try {
        let id = req.body.id;
        let generaldata = {
...
router.put('/api/jobseekereducation', async (req, res) => { // for add education data of jobseeker
    try {
        let id = req.body.id;
...
router.put('/api/jobseekerexperience', async (req, res) => { // for update experience data of jobseeker
    try {
        let id = req.body.id;
...
router.put('/api/onejobseeker', async (req, res) => {    // for admin to get one jobseeker to verify
    try {
        let id = req.body._id;
        let update = {
            name: req.body.name,
...
router.delete('/api/deletejobseeker/:id', async (req, res) => {        // for admin to delete jobseeker
    try {
